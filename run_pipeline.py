"""
Main ETL pipeline orchestrator. Runs the full medallion architecture.
Usage: python run_pipeline.py [--data-dir PATH] [--db-path PATH]
"""
...
def main(data_dir: str, db_path: str):
    ...
    # STEP 1: Bronze - raw load
    t0 = time.time()
    bronze_results = load_bronze(data_dir=data_dir, db_path=db_path)
    log.info(f"Bronze finished in {time.time() - t0:.1f}s")

    # STEP 2: Silver - transformations
    t0 = time.time()
    silver_results = transform_silver(db_path=db_path)
    log.info(f"Silver finished in {time.time() - t0:.1f}s")

    # STEP 3: Gold - analytics layer
    t0 = time.time()
    gold_results = build_gold(db_path=db_path)
    log.info(f"Gold finished in {time.time() - t0:.1f}s")
